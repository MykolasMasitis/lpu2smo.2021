FUNCTION MakeEKMPSV(parra1, parra2, parra3, parra4, parra5)
 ...
 SELECT a.sn_pol FROM talon a, merror b;
	WHERE a.recid=b.recid AND b.et=m.TipOfExp AND b.usr=goApp.smoexp GROUP BY sn_pol ;
		INTO CURSOR curallaccs READWRITE 
 INDEX on sn_pol TAG sn_pol
 SET ORDER TO sn_pol

 SELECT a.sn_pol FROM talon a, merror b;
	WHERE a.recid=b.recid AND b.et=m.TipOfExp AND err_mee!='W0' AND b.usr=goApp.smoexp GROUP BY sn_pol ;
		INTO CURSOR curdefaccs READWRITE 
 INDEX on sn_pol TAG sn_pol
 SET ORDER TO sn_pol
 ...
 *SELECT a.docexp,a.reason as reason FROM merror a, talon b, curokaccs c WHERE ;
 	a.recid=b.recid AND b.sn_pol=c.sn_pol AND ;
 ...
 IF INLIST(m.TipOfExp,'6')
  ...
  m.povod    = IIF(SEEK(m.gcUser+m.theme, 'themes'), themes.name, '')
 ELSE 
 ...
 *SELECT recid, resume, conclusion, recommend FROM svacts WHERE period=m.gcperiod AND mcod=m.mcod AND codexp=INT(VAL(m.TipOfExp)) ;
  AND smoexp=m.usr AND docexp=m.docexp AND flcod=m.flcod INTO CURSOR rqwest NOCONSOLE  
 ...
 ELSE 
   ...
   *INSERT INTO svacts (n_rss,period,mcod,lpu_id,codexp,docexp,flcod,e_period,smoexp,et,qr) ;
    VALUES ;
   (m.n_rss,m.gcperiod,m.mcod,m.lpuid,INT(VAL(m.tipofexp)),goApp.supexp,m.flcod,m.e_period,goApp.smoexp,m.tipofexp,.t.)
   INSERT INTO svacts (n_rss,period,mcod,lpu_id,codexp,docexp,flcod,smoexp,et,qr) ;
    VALUES ;
   (m.n_rss,m.gcperiod,m.mcod,m.lpuid,INT(VAL(m.tipofexp)),goApp.supexp,m.flcod,goApp.smoexp,m.tipofexp,.t.)
 ...
 *m.nrec3 = 0 
 SCAN 
  ...
  IF !(et=m.TipOfExp AND docexp=m.docexp)
   LOOP 
  ENDIF 
  IF !(et=m.TipOfExp AND usr=m.usr)
   LOOP 
  ENDIF 
  ...
  *IF m.er_c == 'GG' AND !SEEK(m.sn_pol, 'curdata3') && ÕÂ ÔÂ‰ÒÚ‡‚ÎÂÌÓ ÔÂ‚Ë˜ÍË
 ...
 *UPDATE svacts SET n_ss=m.checked WHERE recid = m.nfileid
...
*  m.acthead = '1'+PADR(STRTRAN(m.n_akt,'“','T'),16)+PADR(goApp.smoexp,7)+PADR(m.docexp,7)+STRTRAN(m.reason,'0',' ')
  *m.acthead = '1'+PADR(STRTRAN(m.n_akt,'“','T'),16)+PADR(goApp.smoexp,7)+PADR(m.docexp,7)+IIF(m.tipofexp='5', m.reason, ' ')
  m.acthead = '1'+PADR(STRTRAN(m.n_akt,'“','T'),16)+PADR(goApp.smoexp,7)+PADR(goApp.supexp,7)+IIF(m.tipofexp='5', m.reason, ' ')
  m.actbody = ''
