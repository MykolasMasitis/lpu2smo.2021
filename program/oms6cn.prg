 ...
 *m.d_b        = IIF(SEEK(m.lpuid, 'sprnco'), sprnco.datebeg, {}) && {23.03.2020}
 ...
 * Mp = 'm' - МЭСы
  ...
  *m.IsTpnR    = IIF(SEEK(m.cod, 'tarif') AND tarif.tpn='r' AND !(IsKdS(m.cod)), .T., .F.)
  ...
  m.IsTpnR    = IIF(INLIST(m.cod,28211,128211) AND m.IsSprNCO AND BETWEEN(m.d_u,m.d_b,m.d_e), .T., m.IsTpnR)

  m.IsTpnR    = IIF(INLIST(m.cod,28165,128165) AND m.IsIG AND BETWEEN(m.d_u,m.d_b2,m.d_e2), .T., m.IsTpnR) && со счетов за май.
  ...
  m.IsUslGosp = .F.
  IF USED('hosp')
   m.IsUslGosp = IIF(IsUsl(m.cod) AND (SEEK(m.c_i, 'hosp') or m.d_type='s'), .T., .F.)
  ENDIF 
  ...
  *IF IsDental(m.cod, m.lpuid, m.mcod, m.ds) ;
  ...
  DO CASE 
    ...
    *m.Is02 = IIF((SEEK(m.cod, 'tarif') AND tarif.tpn='q') AND m.p_cel='1.1', .T., .F.)
    ...
    DO CASE 
     *CASE m.mcod='0343067' AND IsStac(m.mcod) AND m.IsHor AND m.otd='00'
     * m.Mp = 'p'
