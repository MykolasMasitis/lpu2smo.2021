FUNCTION getAttachmentS(para1, para2, para3) && mailGWlogid!
 IF PARAMETERS() > 0
 ...
 MESSAGEBOX("MSXML2.ServerXMLHTTP",0+64,'getAttachmentS')
 ...
 * Сохраняем http-заголовок
 ...
 IF m.err = .t. 
   ...
   *MESSAGEBOX('ОШИБКА ohttp.responseText!'+CHR(13)+CHR(10)+oEx.Message,0+64,STR(m.httpStatus,3))
   MESSAGEBOX('ОШИБКА ohttp.responseText!'+CHR(13)+CHR(10)+oEx.Message,0+64,'getAttachment')
  ENDIF 
  RETURN .F.
 ENDIF 
