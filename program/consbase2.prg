PROCEDURE ConsBase2
 IF MESSAGEBOX('СОБРАТЬ БАЗЫ ДЛЯ ЭКСПЕРТОВ?',4+32,'2-ОЙ ЭТАП')=7
  RETURN 
 ENDIF 
 ...
 *BROWSE 
 ...
 SCAN 
  ...
  FOR EACH PeriodDir IN PeriodDirs
    ...
    * Открыть аналогичную директорию в главной базе!
    ...
    IF !fso.FileExists(m.pBase+'\'+m.p_name+'\'+m.mcod+'\m'+m.mcod+'.dbf')
     ...
     LOOP 
    ENDIF 
    IF OpenFile(m.pBase+'\'+m.p_name+'\'+m.mcod+'\m'+m.mcod, 'gc_merr', 'shar')>0
     IF USED('gc_merr')
      USE IN gc_merr
     ENDIF 
     USE IN merr
    ...
    * Открыть аналогичную директорию в главной базе!
    ...
    SELECT merr && Нелина база
    SCAN 

     SCATTER FIELDS EXCEPT rid MEMVAR 

     IF !SEEK(m.recid, 'gc_merr', 'recid') && gc_merr - сводная база враче
      *INSERT INTO gc_merr FROM MEMVAR 
     ELSE 
      *IF EMPTY(m.e_period)
      * m.ppp = gc_merr.e_period
      * REPLACE e_period WITH m.ppp
      *ENDIF 
      IF EMPTY(gc_merr.e_period)
       REPLACE e_period WITH m.e_period IN gc_merr
      ENDIF 
     ENDIF 
