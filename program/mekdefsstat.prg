...
*  oExcel.Quit
 ...
 WITH oExcel
  ...
  .Cells(2,2)=' Ó‰ Àœ”'
  .Cells(2,3)=' Ó‰ ‰ÂÙÂÍÚ‡ ÔÓ ‘‘ŒÃ—'
  .Cells(2,4)='Õ‡ËÏÂÌÓ‚‡ÌËÂ ‰ÂÙÂÍÚ‡'
  .Cells(2,5)=' ÓÎ-‚Ó ‰ÂÙÂÍÚÓ‚'
  .Cells(2,6)='—ÛÏÏ‡ ÒÌˇÚËÈ ÔÓ ‰ÂÙÂÍÚÛ'
  .Cells(2,7)='¿œœ'
  .Cells(2,8)='ƒ—“'
  .Cells(2,9)='—“'
  .Cells(2,10)=' Ó‰ ÂÂÒÚ‡'
 ...
 SCAN 
  ...
  SCAN 
   ...
   m.s_all  = talon.s_all
   m.s_app = IIF(m.IsPlk(m.cod), m.s_all, 0)
   m.s_st  = IIF(m.IsGsp(m.cod), m.s_all, 0)
   m.s_dst = IIF(m.IsDst(m.cod), m.s_all, 0)
   m.ok_u   = 0
   m.os_all = 0
   m.os_app = 0
   m.os_st  = 0
   ...
   IF SEEK(m.c_err+m.mcod, 'curdata')
    ...
    m.ns_app = m.os_app + IIF(m.IsPlk(m.cod), m.s_all, 0)
    m.ns_st  = m.os_st  + IIF(m.IsGsp(m.cod), m.s_all, 0)
    m.ns_dst = m.os_dst + IIF(m.IsDst(m.cod), m.s_all, 0)
 ...
 SCAN 
  WITH oExcel
   .Cells(m.n,1) = c_err
   .Cells(m.n,2) = mcod
   .Cells(m.n,3) = sookod.osn230
   .Cells(m.n,4) = sookod.comment
   .Cells(m.n,5) = k_u
   .Cells(m.n,6) = s_all
   .Cells(m.n,7) = s_app
   .Cells(m.n,8) = s_dst
