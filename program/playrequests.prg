PROCEDURE PlayRequests
 IF MESSAGEBOX('ОТПРАВИТЬ ЗАПРОСЫ?',4+32,'')=7
  RETURN 
 ENDIF 
...
FUNCTION SendRequest(para1)
  ...
  *IF  ohttp.status<>200
   ...
   *MESSAGEBOX('ОШИБКА ЗАПРОСА СТАТУС '+STR(ohttp.status)+CHR(13)+CHR(10)+ALLTRIM(ohttp.statusText),0+64,'')
   MESSAGEBOX('ОШИБКА ЗАПРОСА СТАТУС '+STR(m.s_tatus)+CHR(13)+CHR(10)+ALLTRIM(STR(m.s_tatus)),0+64,'')
   RELEASE oXML, oHttp
   IF USED('people')
    USE IN people
   ENDIF 
  ...
  * Сохраняем http-заголовок ответа
  ...
  ENDTRY 
  IF m.err = .t. 
   poi.Write('Произошла ошибка ohttp.responseText! Ошибка в библиотеке msxml3.dll!')
   poi.Close
   RELEASE oXML, oHttp
