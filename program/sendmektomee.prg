PROCEDURE SendMekToMee(para1)
 m.mcod = para1
 IF MESSAGEBOX(CHR(13)+CHR(10)+'ÎÒÎÁÐÀÒÜ Ñ×ÅÒÀ, ÇÀÁÐÀÊÎÂÀÍÍÛÅ ÏÎ ÌÝÊ?'+CHR(13)+CHR(10),4+32,'')=7
  RETURN 
 ENDIF 

 m.lpuid = lpuid
 
 CREATE CURSOR curmek (nrec i AUTOINC, period c(7), lpuid i(4), mcod c(7), sn_pol c(25), c_i c(30), ;
  ...
  d_exp d, docexp c(6),dschet d, nschet c(7), n_dog c(16), c_err c(2))
 ...
 m.dotname = ptempl+'\sh_mek.xls'
 m.docname = pmee+'\mek2mee'+m.qcod+PADL(DAY(DATE()),2,'0')+PADL(MONTH(DATE()),2,'0')
...
FUNCTION selmekone(m.mcod)
  ...
  IF !fso.FileExists(m.lpath+'\'+m.mcod+'\people.dbf')
   RETURN 
  ENDIF 
  IF !fso.FileExists(m.lpath+'\'+m.mcod+'\talon.dbf')
   RETURN 
  ENDIF 
  IF !fso.FileExists(m.lpath+'\'+m.mcod+'\e'+m.mcod+'.dbf')
   RETURN 
  ENDIF 
  IF !fso.FileExists(m.lpath+'\'+m.mcod+'\m'+m.mcod+'.dbf')
   RETURN 
  ENDIF 
  ...
  m.lpuname = IIF(SEEK(m.mcod, 'sprlpu', 'mcod'), sprlpu.fullname, '')
  ...
  ENDIF 
  SCAN 
   IF EMPTY(error.rid) OR INLIST(LEFT(error.c_err,2),'PK','PP')
    LOOP 
   ENDIF 
   
   m.c_err  = LEFT(error.c_err,2)
   m.sn_pol = sn_pol
   m.c_i    = c_i
   m.fam    = people.fam
   m.im     = people.im
...
*   m.usrname = IIF(SEEK(m.usr, 'users', 'name'), ALLTRIM(users.fam)+' '+ALLTRIM(users.im)+' '+ALLTRIM(users.ot), '')
   ...
   IF !IsPat(m.cod)
    INSERT INTO curmek (period,lpuid,mcod,sn_pol,c_i,fam,im,ot,dr,w,;
     d_u,ds,ds_n,ds_2,ds_2_n,ds_3,ds_3_n,otd,otdname,pcod,docname,cod,codname,k_u, s_all, ishod,d_beg, d_end,lpuname,ischked,;
     osn230,osn230_n,koeff,straf,s_1,s_2,codho,usr,usrname,d_exp,docexp,dschet,nschet,n_kd,tip,n_dog,c_err) VALUES ;
     (m.gcperiod,m.lpuid,m.mcod,m.sn_pol,m.c_i,m.fam,m.im,m.ot,m.dr,m.w,;
      ...
      m.dschet,m.nschet,m.n_kd,m.tip,m.n_dog,m.c_err) 
   ELSE 
...
FUNCTION OpFiles()
 IF OpenFile(m.lpath+'\'+m.mcod+'\people', 'people', 'shar', 'sn_pol')>0
  RETURN -1 
 ENDIF 
 IF OpenFile(m.lpath+'\'+m.mcod+'\talon', 'talon', 'shar')>0
  RETURN -1 
 ENDIF 
 IF OpenFile(m.lpath+'\'+m.mcod+'\otdel', 'otdel', 'shar', 'iotd')>0
  RETURN -1 
 ENDIF 
 IF OpenFile(m.lpath+'\'+m.mcod+'\doctor', 'doctor', 'shar', 'pcod')>0
  RETURN -1 
 ENDIF 
 IF OpenFile(m.lpath+'\'+m.mcod+'\e'+m.mcod, 'error', 'shar', 'rid')>0
  RETURN -1 
 ENDIF 
 IF OpenFile(m.lpath+'\'+m.mcod+'\m'+m.mcod, 'merror', 'shar', 'recid')>0
  RETURN -1 
 ENDIF 
 IF fso.FileExists(m.lpath+'\'+m.mcod+'\ho'+m.qcod+'.dbf')
  IF OpenFile(m.lpath+'\'+m.mcod+'\ho'+m.qcod, 'ho', 'shar', 'unik')>0
   RETURN -1 
  ENDIF 
 ENDIF 
RETURN 0
