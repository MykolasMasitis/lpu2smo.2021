 ...
 *SELECT * FROM svacts INTO CURSOR cursvacts READWRITE 
 ...
 **USE IN svacts
 ...
 CREATE CURSOR meexp ;
  (period c(6), e_period c(6), lpuid n(6), mcod c(7), et c(1), docexp c(7), sexp n(11,2), stpn n(11,2), ;
  	s_sank n(11,2), n_akt c(15), d_akt d)

 m.tsexp = 0
 m.tssank = 0
 
 *FOR nperiod=18 TO 0 STEP -1 
 FOR nperiod = 5*12 + tMonth TO 0 STEP -1 
...
*      m.s_sank   = 0
     ...
     DO CASE 
      CASE m.t_akt='SV'
       IF !SEEK(INT(VAL(SUBSTR(m.act,10,6))), 'svacts')
        LOOP 
       ELSE 
        IF svacts.status<>'5' 
         LOOP 
       ...
       ENDIF 
      CASE m.t_akt='SS'
       IF !SEEK(INT(VAL(SUBSTR(m.act,10,6))), 'ssacts')
        LOOP 
       ELSE 
        IF ssacts.status<>'5' 
         LOOP 
     ...
     m.w0exp2 = m.w0exp2 + IIF(m.et='2', m.s_opl, 0)
     m.w0exp3 = m.w0exp3 + IIF(m.et='3', m.s_opl, 0)
     m.w0exp4 = m.w0exp4 + IIF(m.et='4', m.s_opl, 0)
     m.w0exp5 = m.w0exp5 + IIF(m.et='5', m.s_opl, 0)
     m.w0exp6 = m.w0exp6 + IIF(m.et='6', m.s_opl, 0)
     m.w0exp7 = m.w0exp7 + IIF(m.et='7', m.s_opl, 0)
     m.w0exp8 = m.w0exp8 + IIF(m.et='8', m.s_opl, 0)
     m.w0exp9 = m.w0exp9 + IIF(m.et='9', m.s_opl, 0)
...
*     m.d_a = m.actdat
   ...
   USE IN merror
   IF USED('mefile')
    m.recsinme = RECCOUNT('mefile')
    USE IN mefile
    IF m.recsinme=0
     m.IsExpMee = .f.
     IF fso.FileExists(MEFilep+'.dbf')
      fso.DeleteFile(MEFileP+'.dbf')
     ENDIF 
    ENDIF 
   ENDIF 
