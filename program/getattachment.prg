FUNCTION getAttachment(para1, para2, para3) && mailGWlogid!
 IF PARAMETERS() > 0
 ...
 *oClient = CreateClientPump(oXml, '')
 ...
 IF oXML.parseError.errorCode != 0 
  MESSAGEBOX(oXML.parseError.reason,0+64,'getAttachment (1)')
  RELEASE oXML, oHttp
  RETURN .F.
 ENDIF 
 ...
 * This property returns a 4-byte integer.
 ...
 IF m.err = .t. 
  RELEASE oXML, oHttp
  MESSAGEBOX('НЕ УДАЛОСЬ УСТАНОВИТЬ СОЕДИНЕНИЕ!'+CHR(13)+CHR(10)+oEx.Message,0+64,'getAttachment (2)')
  RETURN .F.
 ENDIF 
 ...
 IF m.err = .t. 
  RELEASE oXML, oHttp
  MESSAGEBOX('НЕ УДАЛОСЬ ОТПРАВИТЬ!'+CHR(13)+CHR(10)+oEx.Message,0+64,'getAttachment (3)')
  RETURN .F.
 ENDIF 
 ...
 IF m.err = .t. 
  RELEASE oXML, oHttp
  MESSAGEBOX('ОШИБКА ohttp.status!'+CHR(13)+CHR(10)+oEx.Message,0+64,'getAttachment (4)')
  RETURN .F.
 ENDIF 
 ...
 *IF  ohttp.status<>200
 DO CASE 
  ...
  OTHERWISE 
   MESSAGEBOX('ОШИБКА HTTP:  '+STR(m.httpStatus)+CHR(13), 0+64, 'getAttachment (5)')
   RELEASE oXML, oHttp
   RETURN .F.
 ENDCASE 
 ...
 * Сохраняем http-заголовок
 ...
 IF m.err = .t. 
   ...
   *MESSAGEBOX('ОШИБКА ohttp.responseText!'+CHR(13)+CHR(10)+oEx.Message,0+64,STR(m.httpStatus,3))
   MESSAGEBOX('ОШИБКА ohttp.responseText!'+CHR(13)+CHR(10)+oEx.Message,0+64,'getAttachment (6)')
  ENDIF 
  RETURN .F.
 ENDIF 
...
* MESSAGEBOX(m.start_info,0+64,'')
 ...
 IF EMPTY(m.XmlFromFile)
  IF !m.lIsSilent
   MESSAGEBOX('В ПОЛУЧЕННОМ ОТВЕТЕ XML НЕ ОБНАРУЖЕН!', 0+64, 'getAttachment (7)')
  ENDIF 
  RETURN .F.
 ENDIF 
 ...
 *oXML  = CREATEOBJECT("MsXml2.DOMDocument")
 ...
 IF !oxml.loadXML(m.XmlFromFile)
  RELEASE oXML
  IF !m.lIsSilent
   MESSAGEBOX('НЕ УДАЛОСЬ ЗАГРУЗИТЬ XML ФАЙЛ!', 0+64, 'getAttachment (8)')
  ENDIF 
  RETURN .F.
 ENDIF 
 ...
 *Попытка без сохранения файла выделить из него xml
 ...
 IF m.httpStatus=500
  ...
  IF m.n_recs=0
   RELEASE oXml
   IF !m.lIsSilent
    MESSAGEBOX('В ОТВЕТЕ НИ ОДНОЙ ЗАПИСИ!',0+64,'getAttachment (9)')
   ENDIF 
   RETURN .F.
  ELSE 
 ...
 IF !m.lIsBadLpu
  ...
  IF !m.lIsSilent
   MESSAGEBOX('НЕКОРРЕКТОЕ НАИМЕНОВАНИЕ ФАЙЛА: '+m.fname+CHR(13)+CHR(10)+'ФАЙЛ СОХРАНЕН В '+m.curSoapDir+'\INPUT\', 0+64, 'getAttachment (10)')
  ENDIF 
  RETURN .F.
 ENDIF 
