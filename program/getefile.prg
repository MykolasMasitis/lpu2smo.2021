FUNCTION getEFile(para1, para2, para3) && mailGWlogid!
 IF PARAMETERS() > 0
 ...
 *Попытка без сохранения файла выделить из него xml
 ...
 m.lIsBadLpu = .F.
 IF USED('sprlpu')
  IF LEN(m.fname) = 11 && lpuid
   m.lpu_id = SUBSTR(m.fname,4,4)
   ...
   ENDIF 
  ENDIF 
  IF LEN(m.fname) = 12 && lpuid
   m.lpu_id = SUBSTR(m.fname,5,4)
   IF !SEEK(INT(VAL(m.lpu_id)), 'sprlpu', 'lpu_id')
   ELSE 
    m.lIsBadLpu = .T.
    m.mcod = sprlpu.mcod
