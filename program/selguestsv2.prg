PROCEDURE SelGuestsV2
 ...
 m.dotname = pTempl+'\selguestsv2.xls'
 ...
 m.DocName = m.repdir+'\sg'+m.gcperiod+'ver2.xls'
...
FUNCTION SelGuestsOne(para1)
 ...
 IF OpenFile(m.pbase+'\'+m.lcperiod+'\aisoms', 'aisoms', 'shar')>0
  ...
  RETURN 
 ENDIF 
 IF OpenFile(m.pbase+'\'+m.lcperiod+'\nsi\tarifn', 'tarif', 'shar', 'cod')>0
  IF USED('tarif')
   USE IN tarif
  ENDIF 
  USE IN aisoms
...
FUNCTION SelAlienOne(para1)
 ...
 SCAN 
  ...
  IF OpenFile(pbase+'\'+m.lcperiod+'\'+m.mcod+'\talon', 'talon', 'shar')>0
   ...
   LOOP 
  ENDIF 
  IF OpenFile(pbase+'\'+m.lcperiod+'\'+m.mcod+'\e'+m.mcod, 'error', 'shar', 'rid')>0
   IF USED('error')
    USE IN error
   ENDIF 
   USE IN people
   ...
   LOOP 
  ENDIF 
  IF OpenFile(pbase+'\'+m.lcperiod+'\'+m.mcod+'\m'+m.mcod, 'merror', 'shar', 'rid')>0
   IF USED('merror')
    USE IN merror
   ENDIF 
   USE IN merror
