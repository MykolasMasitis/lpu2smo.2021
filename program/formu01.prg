PROCEDURE FormU01
 IF MESSAGEBOX('СФОРМИРОВАТЬ ФОРМУ Ю-1',4+32,'')=7
  RETURN 
 ENDIF 
 IF !fso.FileExists(pTempl+'\FormU01.xls')
  MESSAGEBOX('ОТСУТСТВУЕТ ФАЙЛ ШАБЛОНА FormU02.xls',0+64,'')
  RETURN 
 ENDIF 
 ...
 CREATE CURSOR curdata (nrec n(7), lpuid n(4), mcod c(7), lpuname c(120), ;
 	col03 n(11,2), col04 n(11,2), col05 n(11,2))
  ...
  DO CASE 
   CASE EMPTY(m.prmcod) && неприкрепленные
    ...
    DO CASE 
     ...
     CASE INLIST(m.otd,'08','70','73') AND IsStac(m.mcod)
      dimdata(3,11) = dimdata(3,11) + IIF(m.IsErr,0,m.s_all) && допуслуги!
      REPLACE Mp WITH '4'
   ...
   CASE m.mcod != m.prmcod && чужие пациенты
    ...
    DO CASE 
     ...
     OTHERWISE 
       ...
       REPLACE n_vmp WITH IIF(EMPTY(n_vmp), '11', n_vmp+' 11')
   ...
   CASE m.mcod != m.prmcods && чужие пациенты
    ...
    DO CASE 
     ...
     OTHERWISE 
       ...
       REPLACE n_vmp WITH IIF(EMPTY(n_vmp), '13', n_vmp+' 13')
 ...
 SELECT curdata 
 COPY TO &pbase\&gcperiod\FormC01
