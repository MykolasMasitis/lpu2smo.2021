 ...
 *LOCAL oHttp AS MSXML2.ServerXMLHTTP  && попробовать!!!
 ...
 *oHttp = CREATEOBJECT("MSXML2.ServerXMLHTTP") && попробовать!!!
 ...
 *IF  ohttp.status<>200
 DO CASE 
  CASE m.httpStatus=200
   *MESSAGEBOX('status=200',0+64,'')
  CASE m.httpStatus=500
   *MESSAGEBOX('status=500',0+64,'')
  OTHERWISE 
   MESSAGEBOX('ОШИБКА HTTP:  '+STR(m.httpStatus)+CHR(13), 0+64, 'getXMLAttachment')
   RELEASE oXML, oHttp
   RETURN .F.
 ENDCASE 
 ...
 * Сохраняем http-заголовок
 ...
 IF m.err = .t. 
  RELEASE oXML, oHttp
  IF !m.lIsSilent
   *MESSAGEBOX('ОШИБКА ohttp.responseText!'+CHR(13)+CHR(10)+oEx.Message,0+64,STR(m.httpStatus,3))
   MESSAGEBOX('ОШИБКА ohttp.responseText!'+CHR(13)+CHR(10)+oEx.Message,0+64,'getXMLAttachment')
  ENDIF 
  *RETURN .F.
 ENDIF 
 
 * пробуем найти посылку в АИС
 IF m.err = .t. 
  m.msg = ALLTRIM(cmessage)
  IF EMPTY(m.msg)
   RELEASE m.msg
   RETURN .F.
  ENDIF 
  IF VARTYPE(CTOD(SUBSTR(SUBSTR(m.msg,1,8),7,2)+'.'+SUBSTR(SUBSTR(m.msg,1,8),5,2)+'.'+;
	SUBSTR(SUBSTR(m.msg,1,8),1,4))) <> 'D'
   RELEASE m.msg
   RETURN .F.
  ENDIF 
  m.dt = SUBSTR(SUBSTR(m.msg,1,8),7,2)+'.'+SUBSTR(SUBSTR(m.msg,1,8),5,2)+'.'+;
	SUBSTR(SUBSTR(m.msg,1,8),1,4)
  ...
  *ENDIF 
  ...
  FOR EACH oFileInDir IN oFilesInDir
   ...
   IF fso.FileExists(pAisOms+'\oms\input\'+dattaches(1,1))
    m.mcod = SUBSTR(dattaches(1,2),2,7)
    fso.CopyFile(pAisOms+'\oms\input\'+dattaches(1,1), ;
    	pBase+'\'+m.gcPeriod+'\'+m.mcod+'\'+dattaches(1,2), .t.)
    *fso.DeleteFile(pAisOms+'\oms\input\'+dattaches(1,1))
    m.mmy = SUBSTR(m.gcPeriod,5,2)+SUBSTR(m.gcPeriod,4,1)
    fso.CopyFile(pAisOms+'\oms\input\'+dattaches(1,1), ;
    	m.pBase+'\'+m.gcPeriod+'\'+m.mcod+'\b'+mailGWlogid+'.'+m.mmy, .t.)
  ...
  IF fso.FileExists(pBase+'\'+m.gcPeriod+'\'+m.mcod+'\'+dattaches(1,2))
   RETURN .T.
  ELSE 
   RETURN .F.
  ENDIF 
 ...
 *Попытка без сохранения файла выделить из него xml
 ...
 m.n_len = oxml.selectNodes('soap:Envelope/soap:Body/ns2:getXMLAttachmentResponse/attachment/attachmentName').length
 IF m.n_len <> 1
 ...
 m.fname    = oxml.selectNodes('soap:Envelope/soap:Body/ns2:getXMLAttachmentResponse/attachment/attachmentName').item(0).text
 m.checkSum = INT(VAL(oxml.selectNodes('soap:Envelope/soap:Body/ns2:getXMLAttachmentResponse/attachment/checkSum').item(0).text))

 IF EMPTY(m.fname)
  MESSAGEBOX('ЗНАЧЕНИЕ attachmentName='+m.fname, 0+64, 'getXMLAttachment')
  RELEASE oXml
  RETURN .T.
 ENDIF 
  ...
  IF !m.lIsSilent
   IF m.crc<>m.checkSum
    MESSAGEBOX('КОНТРОЛЬНАЯ СУММА       ' + STR(m.crc,10)+CHR(13)+CHR(10)+;
    	       'НЕ СОВПАДАЕТ С ИСХОДНОЙ ' + STR(m.checkSum,10), 0+64, 'getXMLAttachment')
   ELSE
*    MESSAGEBOX('КОНТРОЛЬНАЯ СУММА      ' + STR(m.crc,10)+CHR(13)+CHR(10)+;
