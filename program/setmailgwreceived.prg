PROCEDURE setMailGwReceived(para1, para2, para3, para4)
 IF PARAMETERS() > 0
  m.billid    = para1
 ELSE 
  m.billid    = ''
 ...
 * Create a procesing instruction.  
 ...
 oRequest = oXML.createElement("ws:setMailGwReceived")
 ...
 oXXX.appendChild(oXML.createElement("mailGWlogid")).text = m.billid && вероятно, следует поменять на gwlogid!
 ...
 *Попытка без сохранения файла выделить из него xml
 ...
 IF EMPTY(m.XmlFromFile)
  IF !m.lIsSilent
   *MESSAGEBOX('В ПОЛУЧЕННОМ ОТВЕТЕ XML НЕ ОБНАРУЖЕН!', 0+64, IIF(m.lpu_id>0, STR(m.lpu_id,4), 'setMailGwReceived'))
   MESSAGEBOX('В ПОЛУЧЕННОМ ОТВЕТЕ XML НЕ ОБНАРУЖЕН!', 0+64, 'setMailGwReceived')
  ENDIF 
  RETURN .T.
 ENDIF 
 ...
 IF !oxml.loadXML(m.XmlFromFile)
  RELEASE oXML
  IF !m.lIsSilent
   *MESSAGEBOX('НЕ УДАЛОСЬ ЗАГРУЗИТЬ XML ФАЙЛ!', 0+64, IIF(m.lpu_id>0, STR(m.lpu_id,4), 'setMailGwReceived'))
   MESSAGEBOX('НЕ УДАЛОСЬ ЗАГРУЗИТЬ XML ФАЙЛ!', 0+64, 'setMailGwReceived')
  ENDIF 
  RETURN .T.
 ENDIF 
 ...
 *Попытка без сохранения файла выделить из него xml
 ...
 IF m.httpStatus=500
  ...
  ELSE 
    ...
    *MESSAGEBOX('faultcode= '+m.faultcode+CHR(13)+CHR(10)+;
    ...
    MESSAGEBOX('faultcode= '+m.faultcode+CHR(13)+CHR(10)+;
			'faultstring= '+m.faultstring+CHR(13)+CHR(10), 0+64, '')
   ENDIF 
 ...
 m.orec = oxml.selectNodes('soap:Envelope/soap:Body/ns2:setMailGwReceivedResponse/changeBillStatusDTO').item(0)
