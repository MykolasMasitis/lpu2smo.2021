PROCEDURE GospWOLech
 IF MESSAGEBOX(CHR(13)+CHR(10)+'ОТОБРАТЬ ГОСПИТАЛИЗАЦИИ'+CHR(13)+CHR(10)+'БЕЗ ОБСЛЕДОВАНИЯ В НАПРАВИВШЕМ МО?',4+32,'')=7
  RETURN 
 ENDIF 
...
*  m.lcperiod = STR(tYear,4)+PADL(lnmonth,2,'0')
  ...
  m.prperiod = STR(YEAR(GOMONTH(m.tdat1,-1)),4)+PADL(MONTH(GOMONTH(m.tdat1,-1)),2,'0') && предыдущий период
...
*   LOOP 
 ...
 CREATE CURSOR curgosps (period c(7), lpuid i(4), mcod c(7), sn_pol c(25), c_i c(30), ;
  fam c(25), im c(25), ot c(25), dr d, w n(1),d_u d, ds c(6), dss c(3), otd c(4), pcod c(10), ord n(1), date_ord d, lpu_ord n(6),;
...
* ENDFOR 
 ...
 CREATE TABLE &outfile (period c(7), lpuid i(4), mcod c(7), sn_pol c(25), c_i c(30), ;
  ...
  ord n(1), date_ord d, lpu_ord n(6), codname c(40), lpuname c(40), ischked l)
 USE 
...
* SET RELATION TO sn_pol INTO cur_tmpl
 SCAN 
  ...
  IF !SEEK(m.lpu_ord, 'sprlpu')
   LOOP 
  ENDIF 
  m.ord_mcod = sprlpu.mcod
  IF !fso.FolderExists(pBase+'\'+m.lcperiod+'\'+m.ord_mcod)
   LOOP 
  ENDIF 
  IF !fso.FileExists(pBase+'\'+m.lcperiod+'\'+m.ord_mcod+'\talon.dbf')
   LOOP 
  ENDIF 
  IF OpenFile(pBase+'\'+m.lcperiod+'\'+m.ord_mcod+'\talon', 'talon', 'shar', 'sn_pol')>0
   IF USED('talon')
    USE IN talon 
   ENDIF 
   SELECT curgosps
  ...
  IF !fso.FolderExists(pBase+'\'+m.prperiod+'\'+m.ord_mcod)
   LOOP 
  ENDIF 
  IF !fso.FileExists(pBase+'\'+m.prperiod+'\'+m.ord_mcod+'\talon.dbf')
   LOOP 
  ENDIF 
  IF OpenFile(pBase+'\'+m.prperiod+'\'+m.ord_mcod+'\talon', 'talon', 'shar', 'sn_pol')>0
   IF USED('talon')
    USE IN talon 
   ENDIF 
   SELECT curgosps
...
*   ENDIF 
  ...
  SCAN 
   ...
   m.tip = tip
   m.ord = ord
   IF EMPTY(tip) AND INLIST(m.ord,1,2,4)
    LOOP 
   ENDIF 
   ...
   IF !SEEK(m.c_i, 'curgosps')
    INSERT INTO curgosps (period,lpuid,mcod,sn_pol,c_i,fam,im,ot,dr,w,d_u,k_u,n_kd,ds,dss,otd,pcod,d_beg,d_end,cod,s_all,ischked,tip,lpuname,ord,date_ord,lpu_ord) VALUES ;
     (m.gcperiod,m.lpuid,m.mcod,m.sn_pol,m.c_i,m.fam,m.im,m.ot,m.dr,m.w,m.d_u,m.k_u,m.n_kd,m.ds,m.dss,m.otd,m.pcod,m.d_beg,m.d_end,m.cod,m.s_all,m.ischked,m.tip,m.lpuname,m.ord,m.date_ord,m.lpu_ord) 
   ELSE 
     m.ok_u = curgosps.k_u
     m.nk_u = m.ok_u + m.k_u
