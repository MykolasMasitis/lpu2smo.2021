PROCEDURE MakeUDFilesN
 IF MESSAGEBOX(CHR(13)+	CHR(10)+'ÂÛ ÕÎÒÈÒÅ ÑÔÎÐÌÈÐÎÂÀÒÜ UD-ÔÀÉËÛ?'+CHR(13)+CHR(10),4+32,'ÍÎÂÛÉ ÂÀÐÈÀÍÒ')=7
  RETURN 
 ENDIF 
 ...
 SCAN FOR !DELETED()
  ...
  IF !m.IsPilot && Äîáàâèë 26.11.2018!
   IF !SEEK(m.lpuid, 'horlpu')
    * LOOP  && ñäåëàíî äëÿ Êîí÷àëîâñêîãî
   ENDIF 
  ENDIF 
...
*   SELECT prmcod as mcod, SUM(pr_all) as s_all WHERE vz=1 FROM dufile GROUP BY prmcod INTO CURSOR curstat
    ...
    *IF s_all != pr4.s_others + pr4st.s_others
     ...
     *MESSAGEBOX(CHR(13)+CHR(10)+'ÎÁÍÀÐÓÆÅÍÎ ÐÀÑÕÎÆÄÅÍÈÅ ÌÅÆÄÓ ÔÀÉËÎÌ PR4 È'+CHR(13)+CHR(10)+;
      ...
      'ÏÎ ÄÀÍÍÛÌ PR4 ÑÓÌÌÀ S_OTHERS='+TRANSFORM(pr4.s_others + pr4st.s_others,'99999999.99')+CHR(13)+CHR(10)+;
     ...
     *ELSE 
      MESSAGEBOX(CHR(13)+CHR(10)+'ÎÁÍÀÐÓÆÅÍÎ ÐÀÑÕÎÆÄÅÍÈÅ ÌÅÆÄÓ ÔÀÉËÎÌ PR4 È'+CHR(13)+CHR(10)+;
      	'È ÑÔÎÐÌÈÐÎÂÀÍÍÛÌÈ UD-ÔÀÉËÀÌÈ!'+CHR(13)+CHR(10)+;
      	'ÏÎ ÄÀÍÍÛÌ PR4 ÑÓÌÌÀ S_OTHERS='+TRANSFORM(pr4.s_others,'9999999999.99')+CHR(13)+CHR(10)+;
      	'ÏÎ ÄÀÍÍÛÌ UD-ÔÀÉËÎÂ ÑÓÌÌÀ='+TRANSFORM(s_all,'9999999999.99')+CHR(13)+CHR(10),0+48, m.mcod)
     *ENDIF 
    ENDIF 
   ...
   IF m.sumstat!=m.sumpr4
    MESSAGEBOX(CHR(13)+CHR(10)+'ÎÁÍÀÐÓÆÅÍÎ ÐÀÑÕÎÆÄÅÍÈÅ ÌÅÆÄÓ ÔÀÉËÎÌ PR4 È'+CHR(13)+CHR(10)+;
     'È ÑÔÎÐÌÈÐÎÂÀÍÍÛÌÈ UD-ÔÀÉËÀÌÈ!'+CHR(13)+CHR(10)+;
     'ÏÎ ÄÀÍÍÛÌ PR4 ÑÓÌÌÀ S_OTHERS='+TRANSFORM(m.sumpr4,'9999999999.99')+CHR(13)+CHR(10)+;
     'ÏÎ ÄÀÍÍÛÌ UD-ÔÀÉËÎÂ ÑÓÌÌÀ='+TRANSFORM(m.sumstat,'9999999999.99')+CHR(13)+CHR(10),0+48,'')
   ELSE 
    MESSAGEBOX(CHR(13)+CHR(10)+'ÑÓÌÌÀ S_OTHERS ÔÀÉËÀ PR4'+CHR(13)+CHR(10)+;
     'ÑÎÎÒÂÅÒÑÒÂÓÅÒ ÑÓÌÌÅ UD-ÔÀÉËÎÂ'+CHR(13)+CHR(10)+;
     'È ÑÎÑÒÀÂËßÅÒ '+TRANSFORM(m.sumstat,'9999999999.99')+CHR(13)+CHR(10),0+64,'')
   ENDIF 
  ELSE 
   IF USED('pr4')
    USE 
