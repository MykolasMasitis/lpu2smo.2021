PROCEDURE GetPersonInsDataAsyncV01 && GetPersonInsuranceDataMassAsync
 IF MESSAGEBOX('GetPersonInsuranceDataAsync (ver. 01)?',4+32,'SOAP') = 7
  RETURN 
 ENDIF 
 ...
 * Адрес боевого сервиса: http://192.168.192.118:8080/erzl-for-smo/ws/erzlsmowebsvc.wsdl
...
* poi = FOPEN('&curSoapDir\OUTPUT\&xmlFile', 0)
...
* poi = FOPEN('&curSoapDir\OUTPUT\&xmlFile',2)
 poi = FCREATE('&curSoapDir\OUTPUT\&xmlFile')
 IF poi = -1
  MESSAGEBOX('НЕ УДАЛОСЬ СОЗДАТЬ ФАЙЛ: '+CHR(13)+CHR(10)+;
  	'&curSoapDir\OUTPUT\&xmlFile',0+64,'')
  RELEASE oXML, oHttp
  RETURN 
 ENDIF 
 =FSEEK(poi, 0, 2)
