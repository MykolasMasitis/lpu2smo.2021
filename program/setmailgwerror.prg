PROCEDURE setMailGwError(para1, para2, para3, para4, para5)
 IF PARAMETERS() > 0
  m.billid    = para1
 ELSE 
  m.billid    = ''
 ...
 IF PARAMETERS() > 4
  m.c_err = para5
 ELSE 
  m.c_err = 'NSI_OUTDATE' && 'DUBLICATE_PARCEL_WITHOUT_ERROR' 
 ENDIF 
 ...
 * Create a procesing instruction.  
 ...
 oMain = oXML.createElement("ws:setMailGwError")
 oBody.appendChild(oMain)
 
 oClient = CreateClientPump(oXml, '')
 oMain.appendChild(oClient)

 oRequest = oXML.createElement("request")
 oMain.appendChild(oRequest)

 omailErrorIn =  oXML.createElement("mailErrorIn")
 *oRequest.appendChild(omailErrorIn)

 omailErrorIn.appendChild(oXML.createElement("mailGwLogId")).text = m.billid && вероятно, следует поменять на gwlogid!
 omailErrorIn.appendChild(oXML.createElement("errorTypeIn")).text = m.c_err
 ...
 *Попытка без сохранения файла выделить из него xml
 ...
 IF EMPTY(m.XmlFromFile)
  IF !m.lIsSilent
   *MESSAGEBOX('В ПОЛУЧЕННОМ ОТВЕТЕ XML НЕ ОБНАРУЖЕН!', 0+64, IIF(m.lpu_id>0, STR(m.lpu_id,4), 'setMailGwError'))
   MESSAGEBOX('В ПОЛУЧЕННОМ ОТВЕТЕ XML НЕ ОБНАРУЖЕН!', 0+64, 'setMailGwError')
  ENDIF 
  RETURN .T.
 ENDIF 
 ...
 IF !oxml.loadXML(m.XmlFromFile)
  RELEASE oXML
  IF !m.lIsSilent
   *MESSAGEBOX('НЕ УДАЛОСЬ ЗАГРУЗИТЬ XML ФАЙЛ!', 0+64, IIF(m.lpu_id>0, STR(m.lpu_id,4), 'setMailGwError'))
   MESSAGEBOX('НЕ УДАЛОСЬ ЗАГРУЗИТЬ XML ФАЙЛ!', 0+64, 'setMailGwError')
  ENDIF 
  RETURN .T.
 ENDIF 
 ...
 *Попытка без сохранения файла выделить из него xml
 ...
 IF m.httpStatus=500
  ...
  ELSE 
    ...
    *MESSAGEBOX('faultcode= '+m.faultcode+CHR(13)+CHR(10)+;
    ...
    MESSAGEBOX('faultcode= '+m.faultcode+CHR(13)+CHR(10)+;
			'faultstring= '+m.faultstring+CHR(13)+CHR(10), 0+64, 'setMailGwError')
   ENDIF 
 ...
 m.orec = oxml.selectNodes('soap:Envelope/soap:Body/ns2:setMailGwErrorResponse/mailErrorIn').item(0)
  
 m.mailGwLogId   = orec.selectNodes('mailGwLogId').item(0).text
 m.messageStatus = orec.selectNodes('messageStatus').item(0).text
 ...
 IF !m.lIsSilent
  MESSAGEBOX('mailGwLogId='+m.mailGwLogId+CHR(13)+CHR(10)+;
			 'messageStatus= '+messageStatus+CHR(13)+CHR(10)+;
  			 'billid= '+m.billid+CHR(13)+CHR(10)+;
			 'billStatus= '+m.billStatus+CHR(13)+CHR(10)+;
			 'success= '+m.success+CHR(13)+CHR(10), 0+64, 'setMailGwReceived')
 ENDIF 
