FUNCTION MakeEkmpI3n(lcpolis, lcPath, IsVisible, IsQuit, TipAcc, TipOfExp)

 m.usrdir = fso.GetParentFolderName(pbin) + '\'+UPPER(m.gcuser)

 *DotNameWW = 'ActEKMPss.xls'
 DotNameWW = 'EKMPSS_N.xls'
 ...
 *COUNT FOR SEEK(merror.recid, 'talon', 'recid') AND talon.sn_pol = m.sn_pol AND merror.et = m.TipOfExp AND ;
 ...
 COUNT FOR SEEK(merror.recid, 'talon', 'recid') AND talon.sn_pol = m.sn_pol AND merror.et = m.TipOfExp AND ;
 	merror.docexp=m.para7 AND merror.err_mee != 'W0' TO m.nqwert
 ...
 *m.vvir   = m.mcod + m.gcperiod
 ...
 ooal = ALIAS()
 SELECT recid, resume, conclusion, recommend FROM ssacts WHERE period=m.gcperiod AND mcod=m.mcod AND codexp=INT(VAL(m.TipOfExp)) AND ;
...
*  m.d_beg = IIF(IsMes(m.cod) OR IsVMP(m.cod), IIF(m.k_u>1, m.d_u-m.k_u-1, m.d_u-1), m.d_u)
 ...
 m.d_akt = ''
 m.d_exp = {}
 m.zakl_name01 = 'Экспертное заключение №' + m.n_akt + ' от ' + m.d_akt
 ...
 *UPDATE ssacts SET s_all=m.s_lech, s_def=m.nekoplate, s_fee=m.tot_straf, ;
*	n_st=IIF(m.vidp='03',1,0), n_dst=IIF(m.vidp='04',1,0), n_plk=IIF(m.vidp='04',1,0)
* 	WHERE recid = m.nfileid
...
* m.dsname = 'dsname'
 ...
 m.punkt1 = IIF(m.TipOfMee=1,'','')
 m.punkt2 = IIF(m.TipOfMee=1,'НЕТ','')
 m.punkt3 = IIF(m.TipOfMee=1,'БЕЗ ЗАМЕЧАНИЙ','')
 m.punkt4 = IIF(m.TipOfMee=1,'нет','')
 m.punkt5 = IIF(m.TipOfMee=1,'БЕЗ ЗАМЕЧАНИЙ','')
 m.punkt6 = IIF(m.TipOfMee=1,'НЕТ','')
...
* m.llResult = X_Report(m.lcTmpName, m.lcRepName, m.IsVisible)
 ...
 m.str_len = 90
 IF LEN(m.recommend)>0 && если вообще что-то есть
 ...
 *ELSE 
  ...
  * Скрываем Раздел 3  
  ...
  osheet.Cells(25,1).Value="III. Заполняется при наличии данных"
  * Скрываем Раздел 3  

  * Скрываем Раздел 6
  oRange = oExcel.Rows("72:97")
  oRange.EntireRow.Hidden = .t.
  osheet.Cells(71,1).Value="VI. Заполняется при наличии данных"
  * Скрываем Раздел 6  
  ENDIF 
