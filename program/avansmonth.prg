PROCEDURE AvansMonth
 IF MESSAGEBOX('лндскэ гюцпсфюер юбюмянбше окюрефх кос, '+CHR(13)+CHR(10)+;
		    'оепевхякеммше мю якедсчыхи леяъж.'+CHR(13)+CHR(10)+;
            '(гюонкмъеряъ онке S_AVANS2)'+CHR(13)+CHR(10)+;
...
* SET DEFAULT TO (OutDirPeriod)
 ...
 tnresult = 0
 tnresult = tnresult + OpenFile(pbase+'\'+gcperiod+'\aisoms', 'aisoms', 'shared')
 ...
 IF VARTYPE(s_dop1)!='N'
  ...
  USE IN aisoms
  MESSAGEBOX('б тюике '+AvansFile+CHR(13)+CHR(10)+;
   'нрярсрярбсер менаундхлне онке S_DOP1!'+CHR(13)+CHR(10)+;
 ...
 m.notnullavanc = 0
 REPLACE ALL s_avans2 WITH afile.s_dop1
