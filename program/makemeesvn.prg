FUNCTION MakeMEESVn(lcPath, para3, IsQuit, para4, para5) && ¬ÂÒËˇ ‰Îˇ »Ì„ÓÒÒ‡!
 * ¬ÂÒËˇ ÔÓ ÌÓ‚˚È ‡ÍÚ, 22.10.2019
 m.IsVisible = para3
 m.tipofexp  = para4
 ...
 m.nschet    = STR(tYear,4)+PADL(tMonth,2,'0')
 m.dschet    = IIF(FIELD('processed', 'aisoms')='PROCESSED', TTOC(aisoms.processed), '')+', ÌÓÏÂ Ò˜ÂÚ‡ ' + m.nschet
 ...
 *m.docexp = m.paraexp
 ...
 m.vvir   = m.mcod + m.gcperiod + m.TipOfExp
 ...
 *m.saystraf = cpr(INT(m.tot_straf))+' '+PADL(INT((m.tot_straf-INT(m.tot_straf))*100),2,'0')+'  Œœ.'
