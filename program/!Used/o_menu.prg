...
public oMenu
*set path to ..\source
oMenu = newobject('SFMenu', 'libs\SFMenu.vcx', '', 'oMenu')

* Create the File pad.

oMenu.AddPad('SFPad', 'libs\SFMenu.vcx', 'popInfFrLpu')
with oMenu.popInfFrLpu
	.cCaption       = '\<ÈÍÔÎÐÌÀÖÈß ÎÒ ËÏÓ'
	.cKey           = 'ALT+A'
	.cStatusBarText = 'Performs file functions'

	.AddBar('SFBar', 'SFMenu.vcx', 'Bar01')
	with .Bar01
		.cCaption        = 'ÏÐÈÅÌ ÈÍÎÐÌÀÖÈÈ ÎÒ ËÏÓ (ÀÈÑ ÎÌÑ)'
*		.cKey            = 'CTRL+N'
*		.cKeyText        = 'Ctrl+N'
*		.cStatusBarText  = 'Create a file'
		.lInvert          = .T.
		.cPictureFile    = 'newxpsmall.bmp'
		.cOnClickCommand = [DO FORM MailView]
	endwith
