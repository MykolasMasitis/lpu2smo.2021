PROCEDURE PackPrnPdf
 IF MESSAGEBOX('бш унрхре пюяоевюрюрэ юйрш'+CHR(13)+CHR(10)+'на нокюре "ондсьебшу" пюявернб',4+32,'')=7
  RETURN 
 ENDIF 
 ...
 SCAN 
  ...
  IF !fso.FolderExists(pbase+'\'+m.gcperiod+'\'+m.mcod)
   LOOP 
  ENDIF 
  m.docname = '\pdf' + LOWER(m.qcod) + PADL(tMonth,2,'0')+RIGHT(STR(tYear,4),1)
  IF !fso.FileExists(pbase+'\'+m.gcperiod+'\'+m.mcod+'\'+m.docname+'.xlsx')
   LOOP 
  ENDIF 
  ...
  oDoc = oExcel.Workbooks.Add(pbase+'\'+m.gcperiod+'\'+m.mcod+'\'+m.docname+'.xlsx')
  oDoc.PrintOut
  oDoc.Close
